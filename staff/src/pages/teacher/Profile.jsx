...
import {
  User, Mail, Phone, MapPin, Key, LogOut, CheckCircle,
...
export default function TeacherProfile() {
  ...
  const name = user?.teacher?.name || 'Teacher'
  const email = user?.email || ''

  useEffect(() => {
    if (!user?.teacher?.id) { setLoadingDetails(false); return }
    ...
    <DashboardLayout title="My Profile">
      <div className="max-w-2xl mx-auto space-y-5 pb-10">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="h-20 bg-gradient-to-r from-emerald-400/20 via-emerald-300/10 to-transparent" />
          <div className="px-6 pb-6 -mt-8 flex items-end gap-4">
            <ProfilePicUpload accentColor="emerald" />
        ...
        {!loadingDetails && details?.classes?.length > 0 && (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
